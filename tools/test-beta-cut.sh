...
# THE CUT RUNS AGAINST A THROWAWAY CLONE, never this checkout — the same reason
# test-cut-no-publish.sh PART E clones. Two of them, in fact:
#
#   1. This test SEEDS versions/clawee.beta.stamp. Doing that here would have
#      made the suite red for a correct tree the moment an operator opened a
#      real cycle (the file would already exist), and the first version of this
#      test `die`d on exactly that.
#   2. The cut writes dist/<stamp>/. Cleaning that up here means deleting a
#      directory this test does not own — a parked --distribute-only cut lives
#      there.
#
# The clone carries the repo's committed content; the live trees the cut reads
# are copied over it so the case drives the WORKING TREE's code rather than the
# last commit's.
KIT="${WORK}/kit"
git clone -q --no-hardlinks "${REPO_ROOT}" "${KIT}" 2>/dev/null \
    || die "could not clone the repo into a throwaway kit"
for d in tools versions inner cmd internal; do
    cp -R "${REPO_ROOT}/${d}/." "${KIT}/${d}/"
done

BETA_STAMP_FILE="${KIT}/versions/clawee.beta.stamp"
...
STABLE_BEFORE="$(cat "${KIT}/versions/clawee")"
...
ZIP="${KIT}/dist/${STAMP}/clawee-clawee-linux-arm64.zip"
...
[ "$(cat "${KIT}/versions/clawee")" = "${STABLE_BEFORE}" ] \
    || die "the beta cut moved the stable line to $(cat "${KIT}/versions/clawee")"
[ -z "$(git -C "${KIT}" status --porcelain versions/clawee)" ] \
...
[ -z "$(git -C "${KIT}" status --porcelain clawee/install.sh clawee/upgrade.sh)" ] \
